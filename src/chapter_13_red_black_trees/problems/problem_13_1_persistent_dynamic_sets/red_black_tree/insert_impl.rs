...
fn relaxed_insert_left<K: Ord, V>(node: &BlackNode<K, V>, key: Rc<K>, value: Rc<V>) -> RelaxedInsertResult<K, V> {
    match &node.left {
        RedOrBlackNode::Red(left) => match key.cmp(&left.content.key) {
            Ordering::Less => match relaxed_insert(&left.left, key, value) {
                ...
                RelaxedInsertResult::Black(new_left_left, old_value) => {
                    RelaxedInsertResult::Black(node.with_left(left.with_left_rc(Rc::new(new_left_left))), old_value)
                }
            },
            ...
            Ordering::Greater => match relaxed_insert(&left.right, key, value) {
                ...
                RelaxedInsertResult::Black(new_left_right, old_value) => {
                    RelaxedInsertResult::Black(node.with_left(left.with_right_rc(Rc::new(new_left_right))), old_value)
                }
            },
        },
        RedOrBlackNode::Black(left) => match relaxed_insert(left, key, value) {
            RelaxedInsertResult::Red(new_left) => RelaxedInsertResult::Black(node.with_left(Rc::new(new_left)), None),
...
fn relaxed_insert_right<K: Ord, V>(node: &BlackNode<K, V>, key: Rc<K>, value: Rc<V>) -> RelaxedInsertResult<K, V> {
    match &node.right {
        RedOrBlackNode::Red(right) => match key.cmp(&right.content.key) {
            Ordering::Less => match relaxed_insert(&right.left, key, value) {
                ...
                RelaxedInsertResult::Black(new_right_left, old_value) => {
                    RelaxedInsertResult::Black(node.with_right(right.with_left_rc(Rc::new(new_right_left))), old_value)
                }
            },
            Ordering::Equal => RelaxedInsertResult::Black(
